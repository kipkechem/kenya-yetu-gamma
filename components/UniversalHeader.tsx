...
const UniversalHeader: React.FC<UniversalHeaderProps> = ({
    ...
    // Currently Constitution uses the header search bar. Acts/County Laws usually have their own in-page search,
    // but if we want to hoist it, we can. For now, assuming Constitution uses the global one.
        ...
        <header className="flex-shrink-0 z-30 bg-surface/90 dark:bg-dark-surface/90 backdrop-blur-md border-b border-border/50 dark:border-dark-border/50 transition-colors duration-300 sticky top-0 supports-[backdrop-filter]:bg-surface/60">
            <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div className="relative flex items-center justify-between h-16 md:h-20 gap-2 md:gap-4">
                    ...
                    {/* Mobile: Flex-1 (takes available space). Desktop: Absolute Center */}
                    <div className={`
                         ...
                         <div className="w-full transition-all duration-300 ease-out">
                            {showGlobalInput ? (
                                ...
                                <button 
                                    ...
                                    className="flex items-center w-full md:w-full bg-gray-100/80 dark:bg-gray-800/80 border border-gray-200 dark:border-gray-700 hover:border-primary/50 dark:hover:border-dark-primary/50 rounded-full py-2.5 px-4 text-gray-500 dark:text-gray-400 transition-all duration-200 group shadow-sm hover:shadow-md"
                                >
                                    <svg className="w-5 h-5 md:mr-3 text-gray-400 group-hover:text-primary dark:group-hover:text-dark-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                                    <span className="text-sm font-medium hidden md:block">Quick Search...</span>
