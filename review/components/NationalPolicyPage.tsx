...
import { PresentationChartLineIcon, ExternalLinkIcon, ChevronDownIcon } from './icons';
...
const NationalPolicyPage: React.FC = () => {
  ...
  const categories = useMemo(() => {
    if (!policies) return [];
    const cats: string[] = Array.from(new Set(policies.map((p: NationalPolicy) => p.category)));
    ...
    <div className="h-full w-full overflow-y-auto p-4 md:p-6 lg:p-10 bg-background dark:bg-dark-background">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="space-y-10 pb-12">
            {categories.filter((cat: string) => selectedCategory === 'All' || selectedCategory === cat).map((category: string) => {
