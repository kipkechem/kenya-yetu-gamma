            ...
            {searchTerm ? (
              ...
              <div>
                {(Object.keys(actsOfParliament) as Array<keyof ActsByCategory>).map(category => {
                    ...
                    return (
                        <div key={categoryStr} className="border-b border-border dark:border-dark-border last:border-b-0">
                            <button
                            onClick={() => toggleAccordion(categoryStr)}
                            ...
                            aria-expanded={openAccordion === categoryStr}
                            aria-controls={`section-content-${categoryStr}`}
                            >
                            <h3 className="text-lg font-bold text-on-surface dark:text-dark-on-surface">{formatCategoryTitle(categoryStr)} <span className="text-sm font-normal text-gray-500 dark:text-gray-400">({acts.length})</span></h3>
                            <ChevronDownIcon className={`h-6 w-6 text-gray-500 dark:text-gray-400 transform transition-transform duration-300 ${openAccordion === categoryStr ? 'rotate-180' : ''}`} />
                            </button>
                            <div
                            id={`section-content-${categoryStr}`}
                            className={`overflow-hidden transition-all duration-300 ease-in-out bg-background dark:bg-black/20 ${openAccordion === categoryStr ? 'max-h-[3000px]' : 'max-h-0'}`}
                            >
                              <ul className="divide-y divide-border dark:divide-dark-border">
