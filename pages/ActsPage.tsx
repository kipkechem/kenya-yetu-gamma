...
const createSearchUrl = (actTitle: string, categoryKey?: keyof ActsByCategory) => {
  ...
  const params = new URLSearchParams({ 'search[search_word]': actTitle });
  ...
  if (categoryKey && categoryToParam[categoryKey]) {
    ...
    if (categoryKey !== 'in force') params.set('search[seasy_bda]', '0');
  }
            ...
            <div className="flex flex-col">
                <div className="group flex items-center justify-between w-full p-4 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className="p-1.5 rounded-lg bg-primary/10 dark:bg-primary/5 text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                             <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                        </div>
                        <div className="flex-1 min-w-0">
                            <a href={targetUrl} target="_blank" rel="noopener noreferrer" className="font-medium text-sm text-gray-700 dark:text-gray-200 group-hover:text-primary dark:group-hover:text-dark-primary truncate leading-snug hover:underline block">
                                {act.title}
                            </a>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        {hasSubsidiary && (
                            <button onClick={() => setIsExpanded(!isExpanded)} className="p-1.5 rounded-full text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors focus:outline-none">
                                <ChevronDownIcon className={`h-4 w-4 transform transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} />
                            </button>
                        )}
                        <a href={targetUrl} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-full text-gray-400 dark:text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700" aria-label={`View ${act.title}`}>
                        ...
                        <ul className="space-y-1 mb-2">
                            {act.subsidiary!.map((sub, idx) => (
                                <li key={idx} className="flex items-start gap-2 py-1">
                                    ...
                                    <a href={sub.url || createSearchUrl(sub.title, categoryKey)} target="_blank" rel="noopener noreferrer" className="text-sm text-gray-600 dark:text-gray-300 hover:text-primary dark:hover:text-dark-primary hover:underline leading-tight">{sub.title}</a>
                                </li>
                            ))}
                        </ul>
                    </div>
...
const ActsPage: React.FC<ActsPageProps> = ({ searchTerm, onSearchChange }) => {
  ...
  useEffect(() => {
    ...
    if (deferredSearchTerm) setOpenAccordion(null);
  }, [deferredSearchTerm]);
  ...
  if (error) return <ErrorDisplay message="Failed to load Acts." onRetry={refetch} />;
  ...
  return (
    <div className="h-full w-full overflow-y-auto p-4 md:p-6 lg:p-10 bg-background dark:bg-dark-background">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 sticky top-0 py-4 bg-background/95 dark:bg-dark-background/95 backdrop-blur-sm z-10 -mx-4 px-4">
          <div className="relative max-w-2xl mx-auto">
            ...
            <input type="text" placeholder="Search for an Act..." value={searchTerm} onChange={e => onSearchChange(e.target.value)} className="block w-full bg-surface dark:bg-dark-surface border border-border dark:border-dark-border rounded-full py-3.5 pl-12 pr-12 focus:outline-none focus:ring-2 focus:ring-primary custom-shadow-lg" />
          </div>
        </div>
            ...
            <div className="bg-surface dark:bg-dark-surface rounded-2xl custom-shadow-lg overflow-hidden min-h-[400px]">
                ...
                <>
                    <div className="p-4 border-b border-border dark:border-dark-border bg-gray-50/50 dark:bg-white/5">
                        <h3 className="font-semibold text-gray-500 dark:text-gray-400 text-sm uppercase tracking-wider">Search Results ({filteredActs.length})</h3>
                    </div>
                    <ul className="divide-y divide-border dark:divide-dark-border">
                        {filteredActs.slice(0, visibleCount).map((act) => <ActItem key={act.title} act={act} />)}
                    </ul>
                    {filteredActs.length > visibleCount && (
                        <div className="p-6 text-center border-t border-border dark:border-dark-border">
                            <button onClick={handleLoadMore} className="px-6 py-2 bg-primary/10 hover:bg-primary/20 text-primary dark:text-dark-primary rounded-full text-sm font-medium transition-colors">Load More</button>
                        </div>
                    )}
                </>
                ) : (
                <div>
                    {actsOfParliament && CATEGORY_ORDER.map(category => {
                        ...
                        return (
                            <div key={category} className="border-b border-border dark:border-dark-border last:border-b-0">
                                <button onClick={() => setOpenAccordion(isOpen ? null : category)} className={`w-full flex justify-between items-center p-5 text-left transition-colors focus:outline-none ${isOpen ? 'bg-gray-50 dark:bg-white/5' : 'hover:bg-gray-50 dark:hover:bg-white/5'}`}>
                                    <div className="flex items-center gap-3">
                                        <h3 className={`text-lg font-bold ${isOpen ? 'text-primary dark:text-dark-primary' : 'text-on-surface dark:text-dark-on-surface'}`}>{category.charAt(0).toUpperCase() + category.slice(1)}</h3>
                                        <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 bg-gray-200 dark:bg-gray-700 px-2.5 py-0.5 rounded-full">{acts.length}</span>
                                    </div>
                                    <div className={`p-1 rounded-full transition-all duration-300 ${isOpen ? 'bg-primary text-white rotate-180' : 'text-gray-400 bg-gray-100 dark:bg-gray-800'}`}><ChevronDownIcon className="h-5 w-5" /></div>
                                </button>
                                {isOpen && <div className="bg-gray-50/30 dark:bg-black/10 border-t border-border dark:border-dark-border/50 animate-fade-in"><ul className="divide-y divide-border dark:divide-dark-border">{acts.map((act) => <ActItem key={act.title} act={act} categoryKey={category} />)}</ul></div>}
                            </div>
                        );
                    })}
                </div>
